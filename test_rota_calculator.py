...
def test_calcular_rota(mock_astar, mock_nearest, mock_graph):
    ...
    rota = RotaCalculator(mock_graph)
    caminho = rota.calcular_rota(coord_origem=(-7.1000, -34.8000), coord_destino=(-7.2500, -34.9000))
...
@patch("rota_calculator.ox.distance.nearest_nodes", side_effect=ValueError("coordenadas inválidas"))
...
def test_calcular_rota_origem_igual_destino(mock_astar, mock_nearest, mock_graph):
    ...
    rota = RotaCalculator(mock_graph)
    caminho = rota.calcular_rota(coord_origem = (-7.1000, -34.8000), coord_destino = (-7.1000, -34.8000))
