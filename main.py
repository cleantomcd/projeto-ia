...
def executar_rota(nome_cidade, coord_origem, coord_destino):
    """
    Executa o fluxo principal do cálculo de rota.

    Parameters:
        nome_cidade: Nome da cidade.
        coord_origem: Coordenadas da origem (lat, lon).
        coord_destino: Coordenadas do destino (lat, lon).

    Returns:
        tuple: (lista de coordenadas da rota, distância total em km)
    """
    loader = GrafoLoader()
    grafo = loader.carregar_grafo(nome_cidade)
...
if __name__ == "__main__":
    ...
    coordenadas_caminho, distancia_total = executar_rota(cidade, origem, destino)
